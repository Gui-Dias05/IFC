    ...
    <center><h3 style="color: white;">Dashboard dos requisitos</h3></center><br>
        <div class="board-column">
            ...
            <scroll-container>
                <?php
                    ...
                    $consulta = $pdo->query("SELECT requisito.nome, requisito.prazo, requisito.id FROM requisito, usuario_requisito WHERE usuario_requisito.idusu = ".$_SESSION['id']." and requisito.id = usuario_requisito.idreq and requisito.status = 'backlog' group by requisito.id;");
        ...
        <div class="board-column-meio-1">
            ...
            <scroll-container>
                <?php
                    $pdo = Database::iniciaConexao();
                    $consulta = $pdo->query("SELECT requisito.nome, requisito.prazo, requisito.id FROM requisito, usuario_requisito WHERE usuario_requisito.idusu = ".$_SESSION['id']." and requisito.id = usuario_requisito.idreq and requisito.status = 'pending' group by requisito.id;");
                    foreach ($consulta as $linha) { echo"<scroll-page>
                        <span class='board'>
                        Nome: ".$linha['nome']."<br>
                        Prazo: ".date('d/m/Y',strtotime($linha['prazo']))."<hr>
        ...
        <div class="board-column-meio-2">
            ...
            <scroll-container>
                <?php
                    $pdo = Database::iniciaConexao();
                    $consulta = $pdo->query("SELECT requisito.nome, requisito.prazo, requisito.id FROM requisito, usuario_requisito WHERE usuario_requisito.idusu = ".$_SESSION['id']." and requisito.id = usuario_requisito.idreq and requisito.status = 'inprogress' group by requisito.id;");
                    foreach ($consulta as $linha) { echo"<scroll-page>
                        <span class='board'>
                        Nome: ".$linha['nome']."<br>
                        Prazo: ".date('d/m/Y',strtotime($linha['prazo']))."<hr>
        ...
        <div class="board-column-final">
            ...
            <scroll-container>
                <?php
                    $pdo = Database::iniciaConexao();   
                    $consulta = $pdo->query("SELECT requisito.nome, requisito.prazo, requisito.id FROM requisito, usuario_requisito WHERE usuario_requisito.idusu = ".$_SESSION['id']." and requisito.id = usuario_requisito.idreq and requisito.status = 'completed' group by requisito.id;");
                    foreach ($consulta as $linha) { echo"<scroll-page>
                        <span class='board'>
                        Nome: ".$linha['nome']."<br>
                        Prazo: ".date('d/m/Y',strtotime($linha['prazo']))."<hr>
