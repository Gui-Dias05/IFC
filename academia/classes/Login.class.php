<?php
    ...
    class Login extends BancoDados{
        ...
        public function Salvar(){
            try{
                $sql = "INSERT INTO `academia`.`usuario` (`nome`, `email`, `senha`,   `id_funcao` ) 
        ...
        public static function ListarUsuario($tipo = 0, $info = ""){
            try{
                $sql = "SELECT * FROM academia.usuario";
                if($tipo > 0)
                    switch ($tipo) {
        ...
        public static function ListarFuncao($id){
            try{
                $sql = "SELECT * FROM academia.funcao";
                $param = array(); 
                if($id > 0){
        ...
        public function Editar(){
            try{
                $sql = "UPDATE academia.usuario SET nome = :nome, email = :email, senha = :senha WHERE id_usuario = :id";
                $param = array( ":nome" => $this->getNome(),
                                ":email" => $this->getEmail(),
                                ":senha" => $this->getSenha(),
                                ":id" => $this->getId());
        ...
        public function Excluir(){
            try{
                $sql = "DELETE FROM academia.usuario WHERE id_usuario = :id";
        ...
        public static function Logar($email, $senha){
            try{
                if(strlen($email) > 0 && strlen($senha) > 0){
                    $sql = "SELECT * FROM academia.usuario WHERE email = :email AND senha = :senha";
